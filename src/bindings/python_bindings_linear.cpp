...
#include "../linear_model/logistic.h"
...
public:
  ...
  FitOLS &fit(const Eigen::MatrixXd &X, const Eigen::VectorXd &y,
              bool fit_intercept = true, double conf_level = 0.95,
  ...
  Eigen::VectorXd predict(const Eigen::MatrixXd &X, bool fit_intercept = true) {
    if (!is_fitted)
      throw std::runtime_error("Model must be fitted before calling predict");
  ...
  double get_log_likelihood() const {
    return is_fitted ? result.log_likelihood : 0.0;
  }
};
...
PYBIND11_MODULE(linear_model, m) {
  ...
  py::class_<FitOLS>(m, "FitOLS")
      ...
      .def("fit", &FitOLS::fit, py::return_value_policy::reference,
           ...
           py::arg("conf_level") = 0.95, py::arg("gram") = py::none())
      .def("predict", &FitOLS::predict, py::arg("X"),
      ...
      .def_property_readonly("std_errors",
                             [](const FitOLS &self) {
                               return self.is_fitted ? self.result.std_errors
                                                     : Eigen::VectorXd(0);
                             })
      .def_property_readonly("t_values",
                             [](const FitOLS &self) {
                               return self.is_fitted ? self.result.t_values
                                                     : Eigen::VectorXd(0);
                             })
      .def_property_readonly("p_values",
                             [](const FitOLS &self) {
                               return self.is_fitted ? self.result.p_values
                                                     : Eigen::VectorXd(0);
                             })
      .def_property_readonly("conf_int",
                             [](const FitOLS &self) {
                               return self.is_fitted ? self.result.conf_int
                                                     : Eigen::MatrixXd(0, 0);
                             })
      .def_property_readonly("vcov",
                             [](const FitOLS &self) {
                               return self.is_fitted ? self.result.vcov
                                                     : Eigen::MatrixXd(0, 0);
                             })
  ...
  // Logistic Regression Binding
  py::class_<LogisticResult>(m, "LogisticResult")
      .def_readonly("coef", &LogisticResult::coef)
      .def_readonly("intercept", &LogisticResult::intercept)
      .def_readonly("iterations", &LogisticResult::iterations)
      .def_readonly("converged", &LogisticResult::converged)
      .def_readonly("deviance", &LogisticResult::deviance)
      .def_readonly("aic", &LogisticResult::aic);

  py::class_<LogisticRegression>(m, "LogisticRegression")
      .def(py::init<>())
      .def_readwrite("max_iter", &LogisticRegression::max_iter)
      .def_readwrite("tol", &LogisticRegression::tol)
      .def_readwrite("fit_intercept", &LogisticRegression::fit_intercept)
      .def("fit", &LogisticRegression::fit, py::arg("X"), py::arg("y"),
           "Fit logistic regression model")
      .def("predict_prob", &LogisticRegression::predict_prob, py::arg("X"),
           py::arg("coef"), py::arg("intercept") = 0.0,
