...
public:
    ...
    DynamicPanelResult estimate(
        ...
        // For Arellano-Bond, we need rows where difference can be taken (T >= 3 technically for basic, but generally t start from 2 if we have 0,1)
        ...
        // To handle Z efficiently (sparse block diagonal), we might need careful construction.
        // Standard GMM: Y = X B + E.  Z'E = 0.
        // Estimator: B = (X' Z W Z' X)^-1 X' Z W Z' Y
        
        // Let's count usable observations
        int n_diff = 0;
        int T_max = 0;
        for (auto const& [id, obs] : panel_data) {
            if (obs.size() < 2) continue; // Need at least 2 for difference
            n_diff += (obs.size() - 1);
        ...
        // Or better: Use List of Z_i and accumulate cross products directly `Z'X` and `Z'Z`.
        // To implement 1-step W, we need sum(Z_i' H Z_i).
        
        // Count usable instruments to allocate max size?
        // simple AB: for time t (differenced eq for t, t-1), instruments are y_{0}..y_{t-2}
        // Max instruments grows quadratically with T.
        
        // Pass 1: Build Y_diff, X_diff and map rows to (id, t)
        int row_idx = 0;
        std::vector<std::pair<int, int>> row_map; // maps diff_row -> (id, time_of_diff)
        ...
        for (auto const& [id, obs] : panel_data) {
            ...
            for (size_t t = 1; t < obs.size(); ++t) {
                ...
                if (obs[t].first != obs[t-1].first + 1) continue;
                ...
                if (t >= 2) {
                     ...
                     if (obs[t-1].first == obs[t-2].first + 1) {
                         ...
                         // So we effectively need 3 consecutive periods to have 1 obs in regression
                         // (t-2, t-1, t) -> \Delta y_t and \Delta y_{t-1}.
                         continue; 
                     }
                } else {
        ...
        int max_t_used = -1;
        for(auto& p : row_map) {
            if(p.second < min_t_used) min_t_used = p.second;
            if(p.second > max_t_used) max_t_used = p.second;
        }
        ...
        for (int t = min_t_used; t <= max_t_used; ++t) {
            ...
            // Let's keep it simpler: dense-ish Z where cols correspond to specific lags at specific times.
        ...
        // We'll create a full dense Z for valid rows for simplicity of implementation first.
        // Or block-diagonal.
        //
        ...
        // Z = [ Z_{(3)}, 0, ...
        //       0, Z_{(4)}, ... ]
        // Where Z_{(t)} is the stack of instrument vectors for all individuals observed at t.
        // Columns of Z: 
        ...
        for(auto& p : row_map) unique_times.push_back(p.second);
        ...
        for (int t : unique_times) {
            ...
            if (n_instr < 0) n_instr = 0; 
            // Also need to limit by max_lags if set
            if (max_lags > 0) n_instr = std::min(n_instr, max_lags);
        ...
        for (int i = 0; i < N_used; ++i) {
            ...
            int n_instr_this_t = 0;
             ...
             if (max_lags > 0) t_start_instr = std::max(t_start_instr, t_end_instr - max_lags + 1);
             ...
             for (int t_instr = t_start_instr; t_instr <= t_end_instr; ++t_instr) {
                 ...
                 if (found) {
                     ...
                     Z(i, col_start + current_col_offset) = 0.0; // Missing instrument = 0 ?? 
                     // Standard practice: if instrument missing, row might be invalid or we put 0
                     // In GMM, Z should be orthog to error. 0 is safe placeholder if properly handled,
                     // but ideally we only use valid instruments.
                 }
                 current_col_offset++;
             }
        }
        ...
        // For 1-step Difference GMM, H_i is matrix with 2 on diag, -1 on off-diag (MA(1) structure of diff error)
        // (Assuming homoskedasticity)
        // H = Z' * A * Z where A is block diagonal with blocks A_i
        // A_i has 2 on diag, -1 on super/subdiag.
        ...
        std::vector<std::vector<int>> id_to_rows; // id -> list of row indices in Y_diff/X_diff/Z
        ...
        // W1 = (ZAZ)^-1
        // Usually singular if too many instruments. Use Pseudo-Inverse or LDLT with regularization
        Eigen::MatrixXd W1 = ZAZ.ldlt().solve(Eigen::MatrixXd::Identity(total_z_cols, total_z_cols));
        ...
        if (!two_step) {
            ...
            // Usually we assume homoskedasticity for 1-step SE, or use robust formula on 1-step est.
            // Let's calc robust covariance for consistency.
            // Var(b) = (X'Z W Z'X)^-1 X'Z W (sum Z_i' u_i u_i' Z_i) W Z'X (...)
            ...
            Eigen::MatrixXd Q = M.transpose() * W1 * M; // Inverse of this is approx var if optimal W
            // Robust Var = Q^-1 (M' W1 Zuuz W1 M) Q^-1
            Eigen::MatrixXd Q_inv = Q.inverse();
            Eigen::MatrixXd V_robust = Q_inv * (M.transpose() * W1 * Zuuz * W1 * M) * Q_inv;
            ...
            Eigen::MatrixXd W2 = Zuuz.ldlt().solve(Eigen::MatrixXd::Identity(total_z_cols, total_z_cols));
        ...
        // Z/P values
        result.z_values = (result.coefficients.array() / result.std_errors.array()).matrix();
        // naive p-values
