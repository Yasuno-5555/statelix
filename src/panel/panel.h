...
struct PanelStats {
  ...
  static double normal_quantile(double p) {
    ...
    return s *
           std::sqrt(
               std::sqrt((2 / (M_PI * a) + ln / 2) * (2 / (M_PI * a) + ln / 2) -
                         ln / a) -
               (2 / (M_PI * a) + ln / 2)) *
           std::sqrt(2);
  }
...
public:
  ...
  PanelFEResult fit(const Eigen::VectorXd &Y, const Eigen::MatrixXd &X,
    ...
    Eigen::VectorXd Y_demean =
        within_transform(Y, unit_id, time_id, unit_map, time_map);
    Eigen::MatrixXd X_demean(n, k);
    for (int j = 0; j < k; ++j) {
      X_demean.col(j) =
          within_transform(X.col(j), unit_id, time_id, unit_map, time_map);
    }
    ...
    try {
      ...
      throw std::runtime_error(std::string("Solver failed in FE estimation: ") +
                               e.what());
    }

    // Handle Singularity / Rank Deficiency
    // If rank < k, some coefs might be meaningless (already handled by QR/LDLT
    // fallback usually returning a solution) But DoF calculation needs
    // effective rank.
    int rank = solver.rank(); // Exposed in solver.h
    if (rank == 0 && k > 0)
      rank = k; // Fallback if solver didn't set it (shouldn't happen)
    ...
    // If matrix was singular, we only lose rank DoF? Standard stats packages
    // usually penalize for full k or rank. Let's use rank.
    ...
    try {
      ...
      throw std::runtime_error(
          "Could not compute covariance (likely rank deficient FE model): " +
          std::string(e.what()));
    }

    if (cluster_se) {
      result.vcov = compute_cluster_robust_vcov(X_demean, result.residuals,
                                                unit_id, unit_map, XtX_inv);
    } else {
    ...
    for (int j = 0; j < k; ++j)
      result.std_errors(j) = std::sqrt(std::max(0.0, result.vcov(j, j)));
    ...
    double t_crit =
        PanelStats::t_quantile(0.5 + conf_level / 2, result.df_residual);

    for (int j = 0; j < k; ++j) {
      if (result.std_errors(j) > 1e-12) {
        result.t_values(j) = result.coef(j) / result.std_errors(j);
        result.p_values(j) =
            2.0 * (1.0 - PanelStats::t_cdf(std::abs(result.t_values(j)),
                                           result.df_residual));
      } else {
        result.t_values(j) = 0;
        result.p_values(j) = 1;
    ...
    if (two_way)
      result.time_fe = compute_time_fe(Y, X, result.coef, result.unit_fe,
                                       unit_id, time_id, unit_map, time_map);
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (two_way)
        result.fitted_values(i) += result.time_fe(time_map[time_id(i)]);
    }

    if (k > 0) {
      double rss = result.residuals.squaredNorm();
      Eigen::VectorXd Y_demean_null =
          within_transform(Y, unit_id, time_id, unit_map, time_map);
      double tss = Y_demean_null.squaredNorm();
      // F-test logic:
      // ( (TSS - RSS) / rank ) / ( RSS / df_res )
      // Caution: degrees of freedom for numerator is rank (number of
      // predictors).
      result.f_stat = ((tss - rss) / rank) / (rss / result.df_residual);
      result.f_pvalue =
          1.0 - PanelStats::f_cdf(result.f_stat, rank, result.df_residual);
    }
...
private:
  void build_mappings(const Eigen::VectorXi &u_id, const Eigen::VectorXi &t_id,
    ...
    for (int i = 0; i < u_id.size(); ++i) {
      if (u_map.find(u_id(i)) == u_map.end())
        u_map[u_id(i)] = u_map.size();
      if (t_map.find(t_id(i)) == t_map.end())
        t_map[t_id(i)] = t_map.size();
    }
  }

  Eigen::VectorXd within_transform(const Eigen::VectorXd &x,
                                   const Eigen::VectorXi &u_id,
                                   const Eigen::VectorXi &t_id,
                                   const std::unordered_map<int, int> &u_map,
                                   const std::unordered_map<int, int> &t_map) {
    int n = x.size();
    int N = u_map.size();
    ...
    for (int u = 0; u < N; ++u)
      u_mean(u) = (u_cnt(u) > 0) ? u_sum(u) / u_cnt(u) : 0;

    Eigen::VectorXd res(n);
    for (int i = 0; i < n; ++i)
      res(i) = x(i) - u_mean(u_map.at(u_id(i)));
    ...
    if (two_way) {
      ...
      for (int t = 0; t < T; ++t)
        t_mean(t) = (t_cnt(t) > 0) ? t_sum(t) / t_cnt(t) : 0;
      double grand_mean = x.mean();

      for (int i = 0; i < n; ++i) {
        res(i) = x(i) - u_mean(u_map.at(u_id(i))) - t_mean(t_map.at(t_id(i))) +
                 grand_mean;
      }
    }
    return res;
  ...
  Eigen::MatrixXd compute_cluster_robust_vcov(
      const Eigen::MatrixXd &X, const Eigen::VectorXd &resid,
    ...
    for (int g = 0; g < N; ++g)
      meat += cl_sums[g] * cl_sums[g].transpose();

    double corr =
        (double(N) / (N - 1)) * (double(X.rows() - 1) / (X.rows() - k));
    return corr * XtX_inv * meat * XtX_inv;
  }

  void compute_r_squared(const Eigen::VectorXd &Y, const Eigen::MatrixXd &X,
                         const Eigen::VectorXd &beta,
                         const Eigen::VectorXi &u_id,
                         const std::unordered_map<int, int> &u_map,
                         PanelFEResult &res) {
    Eigen::VectorXd Y_demean =
        within_transform(Y, u_id, Eigen::VectorXi::Zero(Y.size()), u_map,
                         std::unordered_map<int, int>{{0, 0}});
    ...
    for (int i = 0; i < Y.size(); ++i)
      fit(i) += res.unit_fe(u_map.at(u_id(i)));
    res.r_squared_overall =
        1.0 - (Y - fit).squaredNorm() / (Y.array() - Y.mean()).square().sum();
  }

  Eigen::VectorXd compute_unit_fe(const Eigen::VectorXd &Y,
  ...
  Eigen::VectorXd
  compute_time_fe(const Eigen::VectorXd &Y, const Eigen::MatrixXd &X,
                  const Eigen::VectorXd &b, const Eigen::VectorXd &u_fe,
                  const Eigen::VectorXi &u_id, const Eigen::VectorXi &t_id,
                  const std::unordered_map<int, int> &u_map,
                  const std::unordered_map<int, int> &t_map) {
    int T = t_map.size();
...
public:
  ...
  PanelREResult fit(const Eigen::VectorXd &Y, const Eigen::MatrixXd &X,
                    const Eigen::VectorXi &u_id, const Eigen::VectorXi &t_id) {
    int n = Y.size();
    int k = X.cols();
    PanelREResult res;
    ...
    try {
      ...
      throw std::runtime_error(std::string("Solver failed in RE estimation: ") +
                               e.what());
    }
    ...
    double t_crit =
        PanelStats::t_quantile(0.5 + conf_level / 2, res.df_residual);
    ...
    for (int j = 0; j < k; ++j) {
      ...
      if (res.std_errors(j) > 1e-12) {
        ...
        res.p_values(j) =
            2.0 * (1.0 - PanelStats::t_cdf(std::abs(res.t_values(j)),
                                           res.df_residual));
      } else {
        res.t_values(j) = 0;
        res.p_values(j) = 1;
    ...
    res.r_squared_overall = 1.0 - res.residuals.squaredNorm() /
...
class HausmanTest {
public:
  static HausmanTestResult test(const PanelFEResult &fe,
                                const PanelREResult &re) {
    ...
    if (neg_eigen) {
      res.warning = "Warning: var_diff matrix is not positive semi-definite. "
                    "Negative eigenvalues clamped.";
      // Standard practice: clamp to small constant
    }
    Eigen::VectorXd evals_clamp = evals.array().max(1e-10);
    Eigen::MatrixXd V_inv =
        evecs * evals_clamp.asDiagonal().inverse() * evecs.transpose();
    ...
    res.recommendation = res.prefer_fe
                             ? "Reject H0: Use Fixed Effects"
                             : "Fail to reject H0: Random Effects consistent";
    return res;
  }

  static HausmanTestResult test(const Eigen::VectorXd &Y,
                                const Eigen::MatrixXd &X,
                                const Eigen::VectorXi &uid,
                                const Eigen::VectorXi &tid) {
    PanelFixedEffects fe;
    fe.cluster_se = false;
 ...
 *
 * Note: Only computes differences for consecutive time periods within each
 * unit. Gaps in time series result in loss of observation for that gap.
 */
...
public:
  ...
  PanelFDResult fit(const Eigen::VectorXd &Y, const Eigen::MatrixXd &X,
                    const Eigen::VectorXi &u_id, const Eigen::VectorXi &t_id) {
    int n = Y.size();
    std::vector<std::tuple<int, int, int>> s_idx;
    for (int i = 0; i < n; ++i)
      s_idx.push_back({u_id(i), t_id(i), i});
    ...
    int k = X.cols();
    if (nd < k)
      throw std::runtime_error("Not enough FD observations.");
    ...
    try {
      ...
      throw std::runtime_error(std::string("Solver failed in FD estimation: ") +
                               e.what());
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Could not compute FD covariance (rank deficiency?)");
    }
    ...
    if (cluster_se) {
      ...
      for (int i = 0; i < nd; ++i)
        if (umap.find(Vdu(i)) == umap.end())
          umap[Vdu(i)] = umap.size();
      int N = umap.size();
      Eigen::MatrixXd meat = Eigen::MatrixXd::Zero(k, k);
      std::vector<Eigen::VectorXd> sums(N, Eigen::VectorXd::Zero(k));
      for (int i = 0; i < nd; ++i)
        sums[umap[Vdu(i)]] += MdX.row(i).transpose() * resid(i);
      for (int g = 0; g < N; ++g)
        meat += sums[g] * sums[g].transpose();
    ...
    for (int j = 0; j < k; ++j) {
      ...
      if (res.std_errors(j) > 1e-12) {
        res.t_values(j) = res.coef(j) / res.std_errors(j);
        res.p_values(j) =
            2.0 * (1.0 - PanelStats::t_cdf(std::abs(res.t_values(j)), nd - k));
      } else {
        res.t_values(j) = 0;
        res.p_values(j) = 1;
      }
    ...
    res.r_squared =
        1.0 - resid.squaredNorm() / (VdY.array() - VdY.mean()).square().sum();
    return res;
  }
};
