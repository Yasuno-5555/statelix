  ...
  WeightedDesignMatrix(const Eigen::MatrixXd &X, const Eigen::VectorXd &weights)
    ...
    if (X.rows() != weights.size()) {
      throw std::invalid_argument(
          "Dimension mismatch: X.rows() != weights.size()");
    }
  }
...
public:
   ...
   * @return 推定された係数 beta
   */
  Eigen::VectorXd solve(const WeightedDesignMatrix &wdm,
  ...
  Eigen::MatrixXd variance_covariance() {
    if (!is_decomposed_) {
      throw std::runtime_error(
          "Solver works must be run before accessing covariance.");
    }
    ...
    if (use_qr_fallback_ || strategy_ == SolverStrategy::QR) {
      ...
      // Standard approach: solve (X^T W X) I = I, but we don't have XtWX in QR
      // explicitly? Actually, qr.solve(I) solves Ax = I? No, QR solves
      // Overdetermined system. But we need (XtWX)^-1. If we have QR of A, then
      // A^T A = R^T R (if Q is orthogonal). So we need (R^T R)^-1. We can
      // compute R from QR object.

      // Simpler fallback for QR covariance:
      // Since we are in the "Robust/Slow" path anyway, getting R and inverting
      // might be okay. Eigen's ColPivHouseholderQR has method to solve for
      // general matrix? If we assume full rank, R is invertible. Better: use
      // the property that solver solves A x = b in least squares sense. We need
      // inverse of Gram matrix. Gram = A^T A. If we successfully solved, we
      // satisfied A x = y. To get (A^T A)^-1, we can solve (A^T A) Z = I. But
      // we only have decomposition of A.
      ...
      // Actually, if we are in QR mode because of Singularity, inverse doesn't
      // exist uniquely! We probably want the Pseudo-Inverse.
      // ColPivHouseholderQR solves for the minimum norm solution? No, basic
      // one.

      // Let's return a generalized inverse approximation or throw if strictly
      // needed. For now: Throw exception for covariance in QR mode or implement
      // later. The user said: "Focus on Cholesky default".
      throw std::runtime_error("Covariance matrix computation for QR fallback "
  ...
  int rank() const {
    ...
    if (use_qr_fallback_ || strategy_ == SolverStrategy::QR)
      return qr_.rank();
    // LDLT doesn't expose strict rank easily like QR, but vectorD can be
    // checked. For now, assume full rank if LDLT succeeded, or P.
    ...
    if (strategy_ == SolverStrategy::QR) {
      ...
      if (strategy_ == SolverStrategy::AUTO && !is_good) {
        ...
        if (used_precomputed) {
          ...
          // Or just fail. User "Correctness" requirement implies we trust
          // inputs or fail.
          throw std::runtime_error(
              "LDLT decomposition failed with precomputed Gram matrix.");
        }
        throw std::runtime_error(
            "LDLT decomposition failed. Matrix might be indefinite.");
      } else {
        use_qr_fallback_ = false;
      }
    }
