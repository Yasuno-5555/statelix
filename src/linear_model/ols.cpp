...
// t分布の累積分布関数の近似（自由度による）
// 簡易実装：正規分布近似を使用（自由度が大きい場合に有効）
double t_cdf_approx(double t, int df) {
    if (df > 30) {
        // 自由度が大きい場合は正規分布で近似
        return 0.5 * (1.0 + std::erf(t / std::sqrt(2.0)));
    }
    // 簡易的なt分布近似（Cornish-Fisher展開の簡易版）
    double z = t / std::sqrt(df / (df - 2.0));
...
OLSResult fit_ols_full(
    ...
    if (mse_resid < 1e-12) {
        ...
        result.f_pvalue = f_pvalue_approx(result.f_statistic, df_model, df_resid);
    }
    ...
    for (int i = 0; i < p_original; ++i) {
        ...
        if (se_all(idx) < 1e-12) {
            ...
            double p_one_sided = 1.0 - t_cdf_approx(t_abs, df_resid);
