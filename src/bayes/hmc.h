...
public:
   ...
   */
  HMCResult sample(EfficientObjective &log_prob_objective,
    ...
    // Storage for warmup samples (ONLY ACCEPTED samples for mass matrix
    // adaptation)
    ...
    for (int iter = 0; iter < total_samples; ++iter) {
      ...
      if (diverged) {
        ...
        if (n_diverge >= config.max_divergences) {
          throw std::runtime_error(
              "Too many divergent transitions (" + std::to_string(n_diverge) +
      ...
      if (is_warmup && config.adapt_step_size) {
        ...
        double accept_prob =
            diverged ? 0.0 : std::min(1.0, std::exp(log_accept_prob));
        int m = iter + 1;
        ...
        double clamped_log_step_size =
            std::max(std::log(config.min_step_size),
        ...
        if (config.adapt_mass_matrix &&
          ...
          if (iter >= window_end) {
            ...
            std::vector<Eigen::VectorXd> window_samples(
                warmup_samples.begin() + window_start, warmup_samples.end());
    ...
    result.step_sizes =
        Eigen::Map<Eigen::VectorXd>(step_sizes.data(), step_sizes.size());
   ...
   */
  HMCResult sample(Objective &log_prob_objective,
                   const Eigen::VectorXd &theta0) {
    // Wrap in adapter
...
private:
   ...
   */
  std::vector<std::pair<int, int>> compute_adaptation_windows(int warmup,
                                                              int init_buffer,
                                                              int term_buffer,
                                                              int base_window) {
   ...
   */
  void leapfrog(EfficientObjective &objective, Eigen::VectorXd &theta,
                Eigen::VectorXd &p, double eps, int L,
                const Eigen::VectorXd &M_inv, bool &diverged) {
    diverged = false;
   ...
   */
  void adapt_mass_matrix(const std::vector<Eigen::VectorXd> &samples,
                         Eigen::VectorXd &M_inv, Eigen::VectorXd &M_sqrt,
                         int dim) {
    if (samples.size() < 10)
      return;
    ...
    Eigen::VectorXd M2 =
        Eigen::VectorXd::Zero(dim); // Sum of squared differences
    ...
    // double reg_scale = 5.0 / (n + 5.0); // Unused
  ...
  void compute_summary(HMCResult &result) {
    ...
    // R-hat: NaN for single chain (requires multiple chains for meaningful
    // estimate) Returning ones would be misleading
    result.r_hat = Eigen::VectorXd::Constant(
        dim, std::numeric_limits<double>::quiet_NaN());
  }

  /**
   * @brief Estimate effective sample size using Geyer's monotone sequence
   * estimator
   *
  ...
  Eigen::VectorXd compute_ess_geyer(const Eigen::MatrixXd &samples) {
    ...
    for (int j = 0; j < dim; ++j) {
      ...
      for (int lag = 1; lag < max_lag - 1; lag += 2) {
        ...
        if (gamma < 0)
          break; // Initial positive sequence stops here
      ...
      double tau =
          -1.0; // Start at -1 because we add rho[0]=1 twice in pair sum
      for (double g : Gamma) {
        tau += g;
      }
      tau = 1.0 + 2.0 * tau;
...
public:
  ...
  HMCResult sample(EfficientObjective &objective,
                   const Eigen::VectorXd &theta0) {
 ...
 */
inline std::vector<HMCResult> run_chains(EfficientObjective &objective,
                                         const Eigen::VectorXd &theta0,
                                         int n_chains = 4, int n_samples = 1000,
                                         int warmup = 500) {
 ...
 */
inline HMCResult hmc_sample(EfficientObjective &objective,
                            const Eigen::VectorXd &theta0, int n_samples = 1000,
                            int warmup = 500) {
  HamiltonianMonteCarlo hmc;
  hmc.config.n_samples = n_samples;
  hmc.config.warmup = warmup;
 ...
 */
inline HMCResult sample_posterior(Objective &neg_log_likelihood,
