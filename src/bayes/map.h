...
#include "../optimization/zigen_optimizer.h" // For Autodiff
#include <Eigen/Dense>
#include <cmath>
#include <memory>
#include <random>
...
public:
  ...
  using ZigenVector = Zigen::Matrix<Var, Zigen::Dynamic, 1>;

  virtual Var value_var(const ZigenVector &x) const {
    throw std::runtime_error("Autodiff not implemented for this prior");
  }
};
...
public:
  ...
  Var value_var(const ZigenVector &) const override { return Var(0.0); }
};
...
public:
  ...
  Var value_var(const ZigenVector &x) const override {
    Var p(precision);
    if (mean.size() > 0) {
      // Convert mean to Zigen vector if needed, or loop
      // x is ZigenVector, mean is Eigen::VectorXd
      // Naive loop for now
      Var val(0.0);
      for (size_t i = 0; i < x.rows(); ++i) {
        Var diff = x(i, 0) - Var(mean(i));
        val = val + diff * diff;
      }
      return Var(0.5) * p * val;
    }
    // Zero mean
    // squaredNorm for Zigen? currently not standard
    Var sum_sq(0.0);
    for (size_t i = 0; i < x.rows(); ++i) {
      sum_sq = sum_sq + x(i, 0) * x(i, 0);
    }
    return Var(0.5) * p * sum_sq;
  }
};
...
public:
  ...
  Var value_var(const ZigenVector &x) const override {
    Var inv_scale = Var(1.0 / scale);
    Var val(0.0);
    using Zigen::Autodiff::abs; // Provided? No, Zigen might not have abs for
                                // Var yet.
    // Reverse.hpp usually has standard math. Let's check or implement
    // simplistic. Assuming user fixes: actually abs() is tricky for
    // differentiable. Usually approximated or handled by Zigen. Let's implement
    // manual abs: (x^2)^0.5

    for (size_t i = 0; i < x.rows(); ++i) {
      Var diff = x(i, 0);
      if (location.size() > 0)
        diff = diff - Var(location(i));

      // Soft abs or standard abs? Laplace uses L1.
      // Zigen Var probably doesn't have 'switch' logic.
      // Using sqrt(diff*diff + eps) for differentiability?
      // Or assume Zigen has abs (if enabled).
      // Let's use sqrt(x*x + 1e-12) for stability
      using Zigen::Autodiff::sqrt;
      Var sq_diff = diff * diff + Var(1e-12);
      val = val + sqrt(sq_diff);
    }
    return val * inv_scale;
  }
};
...
public:
  ...
  Var value_var(const ZigenVector &x) const override {
    ...
    for (size_t i = 0; i < x.rows(); ++i) {
      Var xi_scaled = x(i, 0) / t;
      // log(1 + (x/t)^2)
      val = val + log(Var(1.0) + xi_scaled * xi_scaled);
    }
    return val;
  }
};
...
// =============================================================================
// Zigen Autodiff Objective for MAP
// =============================================================================

class ZigenMAPObjective {
public:
  using Var = Zigen::Autodiff::Reverse::Var;
  using ZigenMatrix = Zigen::Matrix<Var, Zigen::Dynamic, 1>;

  // Likelihood function: returns -log P(D|theta)
  using LikelihoodFunc = std::function<Var(const ZigenMatrix &)>;

  LikelihoodFunc likelihood;
  const Prior &prior;

  ZigenMAPObjective(LikelihoodFunc lik, const Prior &p)
      : likelihood(lik), prior(p) {}

  Var operator()(const ZigenMatrix &theta) const {
    return likelihood(theta) + prior.value_var(theta);
  }
};
...
public:
   ...
   */
  MAPResult estimate(Objective &likelihood, const Eigen::VectorXd &x0,
                     int n_obs = 100) {
    if (!prior) {
    ...
    if (penalizer) {
      ...
      opt_result =
          statelix::minimize(likelihood, x0, penalizer.get(), max_iter);
    } else {
  ...
  MAPResult estimate(
      ...
      const Eigen::VectorXd &x0, int n_obs = 100) {
    LambdaObjective lik_obj(neg_log_lik, grad_neg_log_lik);
    return estimate(lik_obj, x0, n_obs);
  }

  /**
   * @brief Estimate using Zigen Autodiff
   *
   * Users provide a lambda that accepts Zigen types.
   * Gradients and Hessians are computed automatically/semi-automatically.
   */
  MAPResult estimate_autodiff(ZigenMAPObjective::LikelihoodFunc neg_log_lik_var,
                              const Eigen::VectorXd &x0, int n_obs = 100) {
    if (!prior) {
      prior = std::make_unique<FlatPrior>();
    }

    ZigenMAPObjective obj(neg_log_lik_var, *prior);
    ...
    // Zigen Optimization
    OptimizerResult opt_result = optimizer.minimize_autodiff(obj, x0);
    ...
    {
      using Var = Zigen::Autodiff::Reverse::Var;
      Zigen::Matrix<Var, Zigen::Dynamic, 1> z_mode(result.mode.size(), 1);
      for (int i = 0; i < result.mode.size(); ++i)
        z_mode(i, 0) = Var(result.mode(i));

      result.neg_log_likelihood = obj.likelihood(z_mode).val();
      result.neg_log_prior = prior->value(result.mode);
      result.neg_log_posterior =
          result.neg_log_likelihood + result.neg_log_prior;
    }

    if (compute_uncertainty) {
      compute_laplace_approximation_autodiff(obj, result, n_obs);
    }
...
private:
  ...
  public:
    ...
    CompositeObjective(Objective &lik, Prior &p) : likelihood(lik), prior(p) {}
  ...
  public:
    ...
    LambdaObjective(std::function<double(const Eigen::VectorXd &)> v,
  ...
  void compute_laplace_approximation(Objective &likelihood, MAPResult &result,
                                     int n_obs) {
    compute_laplace_common(compute_hessian(likelihood, *prior, result.mode),
                           result, n_obs);
  }

  void compute_laplace_approximation_autodiff(ZigenMAPObjective &obj,
                                              MAPResult &result, int n_obs) {
    compute_laplace_common(compute_hessian_autodiff(obj, result.mode), result,
                           n_obs);
  }

  void compute_laplace_common(const Eigen::MatrixXd &H, MAPResult &result,
                              int n_obs) {
    result.hessian = H;
    ...
    result.bic = 2 * result.neg_log_likelihood +
                 d * std::log(static_cast<double>(n_obs));
  }

  // Numerical Hessian computation
  Eigen::MatrixXd compute_hessian(Objective &likelihood, Prior &prior,
  ...
  // Hessian via Finite Difference of Autodiff Gradients
  Eigen::MatrixXd compute_hessian_autodiff(ZigenMAPObjective &obj,
                                           const Eigen::VectorXd &x) {
    int d = x.size();
    Eigen::MatrixXd H(d, d);
    double eps = 1e-6;

    auto get_grad = [&](const Eigen::VectorXd &params) -> Eigen::VectorXd {
      using Var = Zigen::Autodiff::Reverse::Var;
      Zigen::Matrix<Var, Zigen::Dynamic, 1> theta(d, 1);
      for (int i = 0; i < d; ++i)
        theta(i, 0) = Var(params(i));

      Zigen::Autodiff::Reverse::Tape::getInstance().clear();

      Var loss = obj(theta);

      Zigen::Autodiff::Reverse::backward(loss);
    ...
    for (int j = 0; j < d; ++j) {
      ...
      Eigen::VectorXd g_plus = get_grad(x_plus);
      Eigen::VectorXd g_minus = get_grad(x_minus);

      Eigen::VectorXd col = (g_plus - g_minus) / (2 * eps);
      H.col(j) = col;
    }
 ...
 */
inline MAPResult map_gaussian(Objective &likelihood, const Eigen::VectorXd &x0,
                              double prior_precision = 1.0, int n_obs = 100) {
 ...
 */
inline MAPResult map_laplace(Objective &likelihood, const Eigen::VectorXd &x0,
                             double prior_scale = 1.0, int n_obs = 100) {
  MAPEstimator estimator;
 ...
 */
inline MAPResult mle(Objective &likelihood, const Eigen::VectorXd &x0,
                     int n_obs = 100) {
  MAPEstimator estimator;
