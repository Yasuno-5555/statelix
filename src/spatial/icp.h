...
public:
  ...
  // Aligns Source to Target. Returns transformation R, t such that Target
  // approx R * Source + t Points are rows (N x dim)
  ICPResult align(const Eigen::MatrixXd &source, const Eigen::MatrixXd &target);

  // Optimized alignment using MathUniverse Shinen (Geometric Algebra)
  // Especially efficient for 3D rotations.
  ICPResult align_shinen(const Eigen::MatrixXd &source,
