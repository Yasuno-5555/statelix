...
// Functor must support: double operator()(const Eigen::VectorXd& x,
// Eigen::VectorXd& g)
template <typename Functor> class LBFGS {
public:
  int max_iter = 100;
  ...
  OptimizerResult minimize(Functor &func, const Eigen::VectorXd &x0) {
    ...
    for (iter = 0; iter < max_iter; ++iter) {
      ...
      if (ys > 1e-10) {
        if (s_list.size() >= static_cast<size_t>(m)) {
          s_list.pop_front();
          y_list.pop_front();
          rho_list.pop_front();
        }
...
private:
  ...
  double line_search(Functor &func, Eigen::VectorXd &x,
                     const Eigen::VectorXd &d, double &f, Eigen::VectorXd &g) {
 ...
 * For non-smooth objectives (e.g., with L1 penalty), use ProximalGradient
 * instead.
 */
...
public:
   ...
   */
  OptimizerResult minimize(EfficientObjective &objective,
 ...
 * Design Decisions:
 * -----------------
 * 1. **Step Size Strategy**: Backtracking line search with Armijo-like
 * condition
...
public:
   ...
   * @param x0 Initial parameter vector
   */
  OptimizerResult minimize(Objective &smooth_objective, Penalizer &penalizer,
                           const Eigen::VectorXd &x0) {
    // int n = x0.size(); // Unused
    ...
    for (iter = 0; iter < max_iter; ++iter) {
      ...
      for (int ls = 0; ls < 20; ++ls) {
        ...
        double expected_decrease =
            grad.dot(diff) + (0.5 / step) * diff.squaredNorm() +
 ...
 */
inline OptimizerResult minimize(Objective &objective, const Eigen::VectorXd &x0,
