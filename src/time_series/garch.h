...
public:
    ...
    GARCHObjective(const Eigen::VectorXd& r, GARCHType t, GARCHDist d, int p_, int q_)
    ...
    value_and_gradient(const Eigen::VectorXd& theta) const override {
        ...
        double nll = compute_nll(theta, res); // res is populated with residuals/sigma2
...
private:
    double neg_log_likelihood(const Eigen::VectorXd& theta, GARCHResult& result) const {
        int n = returns.size();
        ...
        if (omega <= 1e-9 && type != GARCHType::IGARCH) return 1e20;
        
        // Vectors
        const double* ptr = theta.data() + 2;
        Eigen::Map<const Eigen::VectorXd> alpha(ptr, p); ptr += p;
        Eigen::Map<const Eigen::VectorXd> beta(ptr, q); ptr += q;
        ...
        if (dist == GARCHDist::STUDENT_T) {
            nu = *ptr;
            if (nu <= 2.01) return 1e20; // Constraint nu > 2
        }
        
        // --- IGARCH Constraint enforcement ---
        // Ideally handled by parameter re-mapping, but for simplified box-constraints:
        if (type == GARCHType::IGARCH) {
            // IGARCH: Sum(alpha) + Sum(beta) = 1
            // Here we assume the optimizer respects bounds? No, L-BFGS is unconstrained.
            // We penalize deviation heavily.
            // Better: Re-parameterize. For now, penalty method.
            double sum = alpha.sum() + beta.sum();
            if (std::abs(sum - 1.0) > 1e-2) return 1e20 + std::pow(sum - 1.0, 2) * 1e6;
        } 
        else if (type == GARCHType::GARCH || type == GARCHType::GJR) {
            // Stationarity check (soft penalty)
            double persistence = alpha.sum() + beta.sum();
            if (n_gamma > 0 && type == GARCHType::GJR) persistence += 0.5 * gamma.sum();
            
            if (persistence >= 0.9999) return 1e20 + (persistence - 0.9999) * 1e6;
            
            // Positivity constraints
            if ((alpha.array() < 0).any() || (beta.array() < 0).any()) return 1e20;
        }
        ...
        for (int t = 0; t < n; ++t) {
            ...
            if (type == GARCHType::EGARCH) {
                 ...
                 for (int i = 1; i <= p; ++i) {
                     if (t - i >= 0) {
                         double z = result.residuals(t - i) / std::sqrt(result.conditional_variance(t - i));
                         double g = alpha(i-1) * z + gamma(i-1) * (std::abs(z) - E_abs_z);
                         ln_sigma2 += g;
                     } else {
                         // Initialization fix: assume z=0 implies g=0?? No, E[g(z)]=0.
                         // So just 0 contribution is correct for expected value, 
                         // but we assume started at stationarity.
                     }
                 }
                 
                 for (int j = 1; j <= q; ++j) {
                     if (t - j >= 0) {
                         ln_sigma2 += beta(j-1) * std::log(result.conditional_variance(t - j));
                     } else {
                         // Initialization fix: Use log(sample_var) instead of just omega
                         // For EGARCH, unconditional log variance is approx omega / (1 - sum(beta))
                         // We use sample estimate
                         ln_sigma2 += beta(j-1) * std::log(sample_var);
                     }
                 }
                 sigma2 = std::exp(ln_sigma2);
                ...
                for (int i = 1; i <= p; ++i) {
                    double eps2;
                    if (t - i >= 0) eps2 = std::pow(result.residuals(t - i), 2);
                    else eps2 = sample_var; // Init with sample var
                    
                    double c = alpha(i-1);
                    if (type == GARCHType::GJR && t - i >= 0 && result.residuals(t - i) < 0) {
                        c += gamma(i-1);
                    }
                    sigma2 += c * eps2;
                }
                ...
                for (int j = 1; j <= q; ++j) {
                    if (t - j >= 0) sigma2 += beta(j-1) * result.conditional_variance(t - j);
                    else sigma2 += beta(j-1) * sample_var;
                }
            }
        ...
        if (std::isnan(log_lik) || std::isinf(log_lik)) return 1e20;
...
public:
    ...
    GARCHResult fit(const Eigen::VectorXd& returns) {
        GARCHResult result;
        result.p = p; result.q = q; result.type = type; result.dist = dist;
        ...
        if (type == GARCHType::IGARCH) theta(1) = 0.0;
        
        // Alphas
        for(int i=0; i<p; ++i) theta(2+i) = 0.05;
        // Betas
        for(int j=0; j<q; ++j) theta(2+p+j) = 0.85/q;
        // Gammas
        if(n_gamma > 0) for(int i=0; i<n_gamma; ++i) theta(n_core+i) = 0.02;
        // Nu
        if(n_dist > 0) theta(n_params-1) = 8.0;
        
        // Check IGARCH constraint initialization
        if (type == GARCHType::IGARCH) {
            // Force sum = 1
            double s = 0;
            for(int k=2; k<2+p+q; ++k) s += theta(k);
            for(int k=2; k<2+p+q; ++k) theta(k) /= s;
        }
        ...
        auto opt_res = optimizer.minimize(obj, theta);
        ...
        // Residuals
        result.standardized_residuals = result.residuals.array() / result.conditional_variance.array().sqrt();
        result.conditional_volatility = result.conditional_variance.array().sqrt();
        
        return result;
    }
    
    GARCHForecast forecast(const GARCHResult& res, int horizon, int n_sims = 1000) {
        GARCHForecast fc;
        fc.horizon = horizon;
        fc.n_sims = n_sims;
        ...
        // Re-implement simplified analytical forecast for point estimate (standard GARCH)
        // And use approximations for bounds if simulation is too heavy to inline here.
        // Actually, user requested simulation. Let's do a proper simpler simulation loop.
        
        std::vector<double> sim_sum_var(horizon, 0.0);
        std::vector<std::vector<double>> sim_vars(horizon);
        for(int h=0; h<horizon; ++h) sim_vars[h].reserve(n_sims);
        ...
        for (int s = 0; s < n_sims; ++s) {
             ...
             for(int i=0; i<p; ++i) local_resid[i] = res.residuals(res.n_obs - 1 - i);
             for(int i=0; i<q; ++i) local_var[i] = res.conditional_variance(res.n_obs - 1 - i);
             for(int i=0; i<p; ++i) local_std_resid[i] = res.standardized_residuals(res.n_obs - 1 - i);
             ...
             for (int h = 0; h < horizon; ++h) {
                 ...
                 if (type == GARCHType::EGARCH) {
                     ...
                     for(int i=1; i<=p; ++i) {
                         ...
                         next_sigma2 += res.alpha(i-1)*z_lag + 
                                        ((res.gamma.size() > 0) ? res.gamma(i-1) : 0) * (std::abs(z_lag) - E_abs_z);
                     }
                     for(int j=1; j<=q; ++j) {
                     ...
                     for(int i=1; i<=p; ++i) {
                         ...
                         if (type == GARCHType::GJR && local_resid[i-1] < 0 && res.gamma.size() > 0) {
                             coef += res.gamma(i-1);
                         }
                 ...
                 // Generate Step
                 double z = (dist == GARCHDist::STUDENT_T) ? t_dist(gen) : normal_dist(gen);
                 if (dist == GARCHDist::STUDENT_T) z *= std::sqrt((res.nu - 2.0)/res.nu);
    ...
    std::pair<Eigen::VectorXd, Eigen::VectorXd> news_impact_curve(
        const GARCHResult& result,
        double z_min = -3.0, double z_max = 3.0, int n_points = 100
    ) {
        ...
        if (!result.is_stationary) sigma2_base = result.conditional_variance.mean(); 
        ...
        for (int i = 0; i < n_points; ++i) {
            ...
            if (type == GARCHType::EGARCH) {
                double E_abs_z = std::sqrt(2.0/M_PI);
                double g = result.alpha(0)*z + ((result.gamma.size())?result.gamma(0):0)*(std::abs(z) - E_abs_z);
                next_s2 = std::exp(result.omega + g + result.beta(0) * std::log(sigma2_base));
            } else {
                double coef = result.alpha(0);
                if (type == GARCHType::GJR && eps < 0 && result.gamma.size()) coef += result.gamma(0);
...
private:
    void unpack_params(const Eigen::VectorXd& theta, GARCHResult& res) {
        ...
        res.omega = theta(1);
        res.alpha.resize(p);
        for(int i=0; i<p; ++i) res.alpha(i) = theta(2+i);
        res.beta.resize(q);
        for(int j=0; j<q; ++j) res.beta(j) = theta(2+p+j);
        
        int offset = 2+p+q;
        if (type == GARCHType::GJR || type == GARCHType::EGARCH) {
            res.gamma.resize(p);
            for(int i=0; i<p; ++i) res.gamma(i) = theta(offset+i);
            offset += p;
        }
        if (dist == GARCHDist::STUDENT_T) res.nu = theta(offset);
        
        // Stats
        res.persistence = res.alpha.sum() + res.beta.sum();
        if (type == GARCHType::GJR && res.gamma.size() > 0) res.persistence += 0.5 * res.gamma.sum();
        
        if (type == GARCHType::IGARCH) res.persistence = 1.0; // By definition
        
        res.is_stationary = (res.persistence < 1.0);
        if (res.is_stationary) res.unconditional_variance = res.omega / (1.0 - res.persistence);
        else res.unconditional_variance = std::numeric_limits<double>::infinity();
        
        if(res.is_stationary) res.half_life = std::log(0.5) / std::log(res.persistence);
        else res.half_life = std::numeric_limits<double>::infinity();
    }
    
    void compute_standard_errors(GARCHObjective& obj, const Eigen::VectorXd& theta, GARCHResult& res) {
        int n = theta.size();
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (int j = i; j < n; ++j) {
                ...
                Eigen::VectorXd pp = theta; pp(i)+=h_i; pp(j)+=h_j;
                Eigen::VectorXd mm = theta; mm(i)-=h_i; mm(j)-=h_j;
                Eigen::VectorXd pm = theta; pm(i)+=h_i; pm(j)-=h_j;
                Eigen::VectorXd mp = theta; mp(i)-=h_i; mp(j)+=h_j;
        ...
        for(int k=0; k<p; ++k) res.alpha_se(k) = se(2+k);
        res.beta_se.resize(q);
        for(int k=0; k<q; ++k) res.beta_se(k) = se(2+p+k);
        
        int offset = 2+p+q;
        if (res.gamma.size() > 0) {
            res.gamma_se.resize(p);
            for(int k=0; k<p; ++k) res.gamma_se(k) = se(offset+k);
            offset += p;
        }
        if (dist == GARCHDist::STUDENT_T) res.nu_se = se(offset);
    }
    
    void shift_vec(std::vector<double>& v, double new_val) {
        for(size_t i=v.size()-1; i>0; --i) v[i] = v[i-1];
        if(!v.empty()) v[0] = new_val;
    }
};
