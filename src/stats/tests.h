...
// Implements: Durbin-Watson, White, Breusch-Pagan, Goldfeld-Quandt, Ramsey
// RESET, Chow, Jarque-Bera, Condition Number, Breusch-Godfrey. Reference:
...
// Using math utilities from stats namespace
using statelix::stats::beta_inc;
using statelix::stats::chi2_cdf;
using statelix::stats::f_cdf;
using statelix::stats::gamma_inc;
 ...
 */
inline DurbinWatsonResult durbin_watson(const Eigen::VectorXd &residuals,
                                        int n_obs, int k) {
  ...
  if (result.dw_statistic < result.d_lower) {
    ...
    result.conclusion = "Negative Autocorrelation";
             ...
             result.dw_statistic < 4 - result.d_upper) {
...
inline BreuschGodfreyResult breusch_godfrey(const Eigen::MatrixXd &X,
  ...
  // Or fill lags with 0 (standard implementation usually drops first p or fills
  // 0) We will fill 0 for lags to keep sample size n (asymtotically equivalent)
  ...
  double tss = residuals.squaredNorm(); // Mean of residuals is 0 by OLS
  ...
  if (denom < 1e-12)
    result.f_statistic = 0; // or inf
  else
    result.f_statistic = num / denom;
...
inline WhiteTestResult white_test(const Eigen::MatrixXd &X,
                                  const Eigen::VectorXd &residuals) {
...
inline BreuschPaganResult breusch_pagan(const Eigen::MatrixXd &X,
                                        const Eigen::VectorXd &residuals) {
...
inline TestResult goldfeld_quandt(const Eigen::MatrixXd &X,
  ...
  result.conclusion = result.reject_null ? "Heteroskedasticity detected"
...
inline RamseyResetResult ramsey_reset(const Eigen::MatrixXd &X,
                                      const Eigen::VectorXd &y,
...
inline ChowTestResult chow_test(const Eigen::MatrixXd &X,
                                const Eigen::VectorXd &y, int break_point) {
  ...
  Eigen::VectorXd beta2 =
      (X2.transpose() * X2).ldlt().solve(X2.transpose() * y2);
 ...
 * @note Uses Thin SVD options to avoid template instantiation errors in sparse
 * context
 */
inline ConditionNumberResult condition_number(const Eigen::MatrixXd &X) {
  ...
  Eigen::JacobiSVD<Eigen::MatrixXd> svd(X_std, Eigen::ComputeThinU |
                                                   Eigen::ComputeThinV);
  ...
  result.condition_number = (min_sv > 1e-12)
                                ? max_sv / min_sv
...
inline TestResult durbin_wu_hausman(const Eigen::VectorXd &y,
  ...
  // OLS augmented
  Eigen::VectorXd beta_aug =
      (X_aug.transpose() * X_aug).ldlt().solve(X_aug.transpose() * y);

  Eigen::VectorXd e = y - X_aug * beta_aug;
  double s2 = e.squaredNorm() / (n - X_aug.cols());

  Eigen::MatrixXd XtX_inv =
      (X_aug.transpose() * X_aug)
          .ldlt()
  ...
  result.reject_null = (result.p_value < 0.05);
  result.conclusion = result.reject_null ? "Endogeneity detected: use IV"
...
inline Eigen::MatrixXd robust_vcov(const Eigen::MatrixXd &X,
                                   const Eigen::VectorXd &residuals,
...
inline Eigen::MatrixXd newey_west_vcov(const Eigen::MatrixXd &X,
                                       const Eigen::VectorXd &residuals,
                                       int max_lag = -1) {
  int n = X.rows();
  int k = X.cols();
  ...
  if (max_lag < 0) {
    max_lag =
        static_cast<int>(std::floor(4.0 * std::pow(n / 100.0, 2.0 / 9.0)));
  }
  ...
  for (int j = 1; j <= max_lag; ++j) {
    ...
    for (int t = j; t < n; ++t) {
      Gamma_j +=
          residuals(t) * residuals(t - j) * X.row(t).transpose() * X.row(t - j);
    }
...
inline Eigen::MatrixXd cluster_robust_vcov(const Eigen::MatrixXd &X,
                                           const Eigen::VectorXd &residuals,
