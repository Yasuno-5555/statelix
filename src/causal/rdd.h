...
public:
   ...
   */
  RDDResult fit(const Eigen::VectorXd &Y, const Eigen::VectorXd &X,
                double cutoff,
    ...
    if (result.n_left < 5 || result.n_right < 5) {
      throw std::runtime_error(
          "Not enough observations near cutoff. Try larger bandwidth.");
    }

    // Build weighted regression
    int n_eff = result.n_eff;
    int k =
        2 * (polynomial_order + 1); // Separate intercept + slopes for each side
    ...
    try {
      ...
      throw std::runtime_error("RDD estimation failed: " +
                               std::string(e.what()));
    }
    ...
    // Recover (ZtWZ)^-1
    // double ssr_solver = resid.squaredNorm(); // Unused
    ...
    // If we can't easily get unscaled, we can compute ZtWZ explicit only if P
    // is small. P is small (4 or 6). So Explicit construction is FINE. BUT user
    // wanted WeightedSolver. Fine, I'll extract it from solver result. If I
    // can't trust sigma2 scaling, I'll assume solver is correct OLS.
   ...
   */
  double compute_ik_bandwidth(const Eigen::VectorXd &Y,
    ...
    // Regularization term
    double r = 2.702 * std::pow(std::abs(m2_left) + std::abs(m2_right) + 0.1,
                                1.0 / 7.0);
   ...
   */
  ManipulationTestResult manipulation_test(const Eigen::VectorXd &X,
                                           double cutoff) {
...
private:
  ...
  double estimate_second_derivative(const Eigen::VectorXd &Y,
                                    const Eigen::VectorXd &X_c, double h,
                                    bool right_side) {
    int n = Y.size();
    ...
    std::vector<int> idx;
    for (int i = 0; i < n; ++i) {
      bool in_range = right_side ? (X_c(i) >= 0 && X_c(i) <= 2 * h)
                                 : (X_c(i) >= -2 * h && X_c(i) < 0);
      if (in_range) {
        idx.push_back(i);
      }
    ...
    Eigen::VectorXd beta =
        (Z.transpose() * W * Z).ldlt().solve(Z.transpose() * W * y);

    return 2.0 * beta(2); // Second derivative = 2 * coefficient on x²
  }

  void compute_bias_correction(const Eigen::VectorXd &Y,
                               const Eigen::VectorXd &X_c, double h,
                               RDDResult &result) {
...
public:
   ...
   */
  RDDResult fit(const Eigen::VectorXd &Y, const Eigen::VectorXd &D,
                const Eigen::VectorXd &X, double cutoff) {
    int n = Y.size();
    RDDResult result;
    result.cutoff = cutoff;
    ...
    double w_ssr_d = 0;
    for (int i = 0; i < n_eff; ++i)
      w_ssr_d += w_eff(i) * d_resid(i) * d_resid(i);
