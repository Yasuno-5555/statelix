...
struct MatchingResult {
  ...
  std::vector<std::vector<int>>
      matches; // For each treated, matched control indices
  std::vector<std::vector<int>>
      matches_atc; // For ATC: each control's matched treated
...
public:
  ...
  query(double query, int k,
        double max_dist = std::numeric_limits<double>::infinity(),
        const std::unordered_set<int> *exclude = nullptr) const {
    if (sorted_.empty())
      return {};
    ...
    while (result.size() < static_cast<size_t>(k)) {
      ...
      double min_possible =
          std::min(has_left ? std::abs(left->first - query)
                            : std::numeric_limits<double>::infinity(),
                   has_right ? std::abs(right->first - query)
                             : std::numeric_limits<double>::infinity());
      if (min_possible > max_dist && result.size() >= static_cast<size_t>(k))
        break;
    }
   ...
   */
  std::vector<std::pair<double, int>>
  radius_query(double query, double radius,
               const std::unordered_set<int> *exclude = nullptr) const {
    if (sorted_.empty())
      return {};
...
public:
   ...
   */
  PropensityScoreResult estimate_propensity(const Eigen::VectorXd &D,
    ...
    // Using AUTO strategy to fallback to QR if XT W X is singular (e.g.
    // collinearity)
   ...
   */
  MatchingResult match(const Eigen::VectorXd &Y, const Eigen::VectorXd &D,
                       const Eigen::MatrixXd &X,
    ...
    if (caliper < 0) {
      ...
      double sd_ps =
          std::sqrt((ps.scores.array() - ps.scores.mean()).square().mean());
    ...
    estimate_effect_atc(Y, control_idx, result.matches_atc, ps.scores, atc,
                        atc_var);
    result.atc = atc;
   ...
   */
  IPWResult ipw(const Eigen::VectorXd &Y, const Eigen::VectorXd &D,
                const PropensityScoreResult &ps) {
    int n = Y.size();
    IPWResult result;
    ...
    // double p_treated = sum_d / n; // Unused
   ...
   *
   * AIPW ATT = E[D(Y - μ₀(X)) / P(D=1)] + E[(1-D)e(X)/(1-e(X)) * (Y - μ₀(X)) /
   * P(D=1)]
   */
  AIPWResult aipw(const Eigen::VectorXd &Y, const Eigen::VectorXd &D,
                  const Eigen::MatrixXd &X, const PropensityScoreResult &ps) {
    int n = Y.size();
    int k = X.cols();
    AIPWResult result;
    ...
    Eigen::VectorXd beta1 =
        (X1.transpose() * X1).ldlt().solve(X1.transpose() * Y1);
    ...
    Eigen::VectorXd beta0 =
        (X0.transpose() * X0).ldlt().solve(X0.transpose() * Y0);
    ...
    // ATT = (1/n₁) Σ_i [D_i(Y_i - μ₀(X_i)) - (1-D_i)e(X_i)/(1-e(X_i))(Y_i -
    // μ₀(X_i))]
    double sum_att = 0;
    int n_t = 0;
    ...
    // ATE = (1/n) Σ_i [μ₁(X_i) - μ₀(X_i) + D_i(Y_i-μ₁)/e -
    // (1-D_i)(Y_i-μ₀)/(1-e)]
    double sum_ate = 0;
    int n_valid = 0;
    ...
    // Variances
    auto compute_var = [](const std::vector<double> &psi, double mean,
                          int denom) {
      double sum2 = 0;
      for (double p : psi) {
    ...
    result.efficiency_gain =
        (ipw_result.att_se > 0)
            ? (ipw_result.att_se - result.att_se) / ipw_result.att_se * 100
            : 0;
...
private:
  ...
  void match_units(
      ...
      const std::vector<int>
          &target_idx, // Pool to match from (control for ATT)
      const SortedIndex1D &target_index, std::vector<std::vector<int>> &matches,
      std::unordered_set<int> &used_targets, double caliper_dist) {
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic)
    ...
    for (size_t t = 0; t < query_idx.size(); ++t) {
      ...
      if (method == MatchingMethod::RADIUS) {
        ...
        neighbors = target_index.radius_query(
            ps_i, r, with_replacement ? nullptr : &used_targets);
      } else if (method == MatchingMethod::KERNEL) {
        // Kernel: get all targets within bandwidth
        neighbors =
            target_index.radius_query(ps_i, kernel_bandwidth * 3, // 3 sigma
                                      nullptr); // Kernel always uses all
      } else {
        // NEAREST_NEIGHBOR or CALIPER
        neighbors =
            target_index.query(ps_i, n_neighbors, caliper_dist,
   ...
   */
  void estimate_effect(const Eigen::VectorXd &Y,
                       const std::vector<int> &treated_idx,
                       const std::vector<std::vector<int>> &matches,
                       const Eigen::VectorXd &scores, double &effect,
                       double &variance) {
   ...
   */
  void estimate_effect_atc(const Eigen::VectorXd &Y,
  ...
  void compute_balance(const Eigen::MatrixXd &X, const Eigen::VectorXd &D,
  ...
  void compute_balance_after(const Eigen::MatrixXd &X, const Eigen::VectorXd &D,
                             const std::vector<std::vector<int>> &matches,
                             const std::vector<int> &treated_idx,
                             const std::vector<int> &control_idx,
                             Eigen::VectorXd &std_diff) {
...
public:
   ...
   */
  std::vector<std::vector<int>> match(const Eigen::MatrixXd &X,
