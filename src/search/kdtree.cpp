...
// Optimized in-place build: O(N log N) time, O(1) extra space per node (stack
// only)
...
KNNSearchResult KDTree::query(const Eigen::VectorXd &point, int k) {
  ...
  for (auto &p : heap) {
    distances.push_back(
        std::sqrt(p.first)); // Convert squared dist to euclidean
...
void KDTree::search_recursive(const KDNode *node, const Eigen::VectorXd &point,
                              int k,
                              std::vector<std::pair<double, int>> &heap) {
  if (!node)
    return;
  ...
  double dist_sq =
      (data.row(node->point_index) - point.transpose()).squaredNorm();
  ...
  // Let's just use vector and push_back + sort approach for simplicity unless
  // slow. Optimization: Keep sorted vector.
  ...
  if (heap.size() < (size_t)k || dist_to_plane_sq < heap.front().first) {
